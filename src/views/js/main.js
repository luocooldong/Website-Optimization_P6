...
// 该字符串变量记录用户上次选择披萨的大小, 当披萨尺寸大小被调整时，这个值会会更新
var latestPizzaSize = "2";

// 记录页面上一次滑动到的位置 ，初始值设为0，当页面滑动时，这个值会更新
var latestKnownScrollTop = 0;

// 该布尔值的状态用来表示当前页是否在用requestAnimationFrame更新的过程中
var currentlyAnimating = false;


// 当resizePizzas函数被调用时，通过requestAnimationFrame的判断间接调用此函数来重置披萨的大小
function updatePizzaSize(){
  ...
  // 遍历披萨的元素并改变它们的宽度   这段代码很疯狂，执行的太多了
  function changePizzaSizes() {
    ...
    var randomPizzas = document.getElementsByClassName(".randomPizzaContainer"),
        numberOfElements = randomPizzas.length; 
...
// 参考链接 使用requestAnimationFrame  https://www.html5rocks.com/en/tutorials/speed/animations/

// 判断updateBackgroundPizzas参数携带的布尔值，
function requestAnimation(updateBackgroundPizzas) {
  if(!currentlyAnimating) {
    // 当为ture时，代表用户的行为是滚动操作，
    if(updateBackgroundPizzas) {
      window.requestAnimationFrame(updatePositions);
    }
    else {
      //当为false时，代表用户的行为是滑块调整pizza大小操作，
...
// 当网站中"Our Pizzas"的滑窗部分移动时调用resizePizzas(size)函数 ,从Html页面中来onchange函数
function resizePizzas(size) {
  latestPizzaSize = size;
  requestAnimation(false);
}

// 在页面滚动时，记录页面最新滚动到的位置，同时调用requestAnimation去更新背景中的pizzas
function onScroll() {
...
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8;
  var s = 256;
  var numbers = (window.outerWidth/s)*cols;
